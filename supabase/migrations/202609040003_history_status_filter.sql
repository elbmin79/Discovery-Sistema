create or replace function public.query_pickup_history(range_from date, range_to date, page_limit int, page_offset int, live_rows jsonb, live_lates jsonb, pickup_status text, pickup_zone text)
returns jsonb
...
as $$
  with all_candidates as (
    ...
    select * from all_candidates where (pickup_status = 'all'
      or (pickup_status = 'active' and record->>'status' not in ('delivered', 'cancelled'))
      or record->>'status' = pickup_status)
      and (pickup_zone = '' or pickup_zone = any(string_to_array(record->>'zoneName', ', ')))
  ), totals as (
...
$$;
revoke all on function public.query_pickup_history(date, date, int, int, jsonb, jsonb, text, text) from public, anon, authenticated;
grant execute on function public.query_pickup_history(date, date, int, int, jsonb, jsonb, text, text) to service_role;
