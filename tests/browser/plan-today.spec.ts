...
test("family sees, edits, scans and completes today's prepared plan", async ({ page }) => {
  ...
  await expect(page.getByRole("button", { name: "Crear recogida", exact: true })).toBeVisible({ timeout: 5000 });
...
test("today plan is fully translated to English", async ({ page }) => {
  ...
  await expect(page.getByText("Emiliano", { exact: true })).toBeVisible();
  await expect(page.getByText("Isabela", { exact: true })).toBeVisible();
  await expect(page.getByText("Paula", { exact: true })).toBeVisible();
});
