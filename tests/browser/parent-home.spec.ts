...
test("cancel a plan, see the home and school contact, then create a new pickup", async ({ page }) => {
  ...
  await expect(page.getByText("Avisos de la escuela", { exact: true })).toBeVisible();
  await expect(page.getByRole("link", { name: /Llamar a la escuela/ })).toHaveAttribute("href", "tel:+526868378517");
  ...
  await expect(page.getByRole("link", { name: /Call the school/ })).toBeVisible();
