...
import { AlarmClock, Car, ClipboardList, Eraser, Info, Undo2, Users, X } from "lucide-react";
...
export function DismissalBoard({
  ...
  const simRunning = Boolean(snapshot?.simulation?.running);

  async function toggleContinuousSim() {
    setTogglingSim(true);
    try {
      await postJson("/api/demo/simulate", { running: !simRunning });
    } finally {
      setTogglingSim(false);
    }
  }

  async function clearArrivals(mode: "simulated" | "all") {
    setClearing(true);
    try {
      await postJson("/api/demo/clear", { mode });
      setShowClear(false);
    } finally {
      setClearing(false);
    }
  ...
  if (syncError && !snapshot) {
    return (
      <main className="flex min-h-dvh items-center justify-center bg-cream">
        <SystemStatus kind="error" context="personal" detail={syncError} onRetry={retry} />
      </main>
    );
  }
  ...
  return (
    <div className="flex min-h-dvh flex-col bg-cream">
      <header className="relative flex flex-wrap items-center justify-between gap-3 border-b border-line bg-paper px-4 py-3 md:px-6">
        ...
        <div className="order-last flex w-full flex-wrap items-center justify-center gap-2 md:order-none md:absolute md:left-1/2 md:top-1/2 md:w-auto md:-translate-x-1/2 md:-translate-y-1/2">
          <button
            type="button"
            ...
            className={`inline-flex items-center gap-2 rounded-full px-4 py-2 text-sm font-semibold transition disabled:opacity-60 ${
              simRunning
                ...
                : "border border-line bg-paper text-forest hover:border-forest/40"
            }`}
            title="Simulación continua de llegadas (compartida en todos los tableros)."
          >
            <span
              className={`h-2.5 w-2.5 rounded-full ${
                simRunning ? "sim-light-on bg-[#9fd4b3]" : "bg-danger"
              }`}
              aria-hidden
            />
            {simRunning ? "Simulando llegadas" : "Simular llegadas"}
          </button>
          <button
            type="button"
            onClick={addRandomArrivals}
            disabled={simulating}
            className="rounded-full border border-dashed border-line px-3 py-2 text-xs font-semibold text-muted transition hover:border-gold-deep hover:text-gold-deep disabled:opacity-60"
            title="Agrega un lote de llegadas de una sola vez."
          >
          ...
          <button
            ...
            className="inline-flex items-center gap-1.5 rounded-full border border-line px-3 py-2 text-xs font-semibold text-muted transition hover:border-danger/40 hover:text-danger"
            title="Limpiar llegadas del tablero."
          >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-ink/40 p-4">
          <div className="w-full max-w-md rounded-2xl border border-line bg-paper p-5 shadow-xl">
            <div className="flex items-start justify-between gap-3">
              <div>
                <h2 className="font-serif text-2xl text-forest">Limpiar tablero</h2>
                <p className="mt-1 text-sm text-muted">Elige qué quieres quitar de la fila.</p>
              </div>
              <button
                type="button"
            ...
            <div className="mt-5 flex flex-col gap-2">
              <button
                type="button"
                disabled={clearing}
                onClick={() => void clearArrivals("simulated")}
                className="rounded-xl border border-line px-4 py-3 text-left text-sm font-semibold text-forest transition hover:border-forest/40 disabled:opacity-60"
              >
                Solo simulados
                <span className="mt-0.5 block text-xs font-normal text-muted">
                  Quita familias sintéticas del simulador continuo.
                </span>
              </button>
              <button
                type="button"
                disabled={clearing}
                onClick={() => void clearArrivals("all")}
                className="rounded-xl border border-danger/30 bg-danger/5 px-4 py-3 text-left text-sm font-semibold text-danger transition hover:bg-danger/10 disabled:opacity-60"
              >
                Limpiar todo
                <span className="mt-0.5 block text-xs font-normal text-danger/80">
                  Incluye recogidas de cuentas de familias reales.
                </span>
              </button>
              <button
                type="button"
                disabled={clearing}
                onClick={() => setShowClear(false)}
                className="mt-1 rounded-full px-4 py-2 text-sm text-muted"
              >
                Cancelar
              </button>
            </div>
