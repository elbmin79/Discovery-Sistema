...
4. Con esas variables inyectadas en el proceso, ejecutar `npm run verify:supabase`. El despliegue no debe promoverse si falla: valida la fila de estado, las tablas de histórico y las funciones de escritura y consulta sin modificar datos.
5. Ejecutar `npm run history:backfill` antes de reanudar tráfico. El comando no carga archivos `.env` por su cuenta. Convierte primero todas las fotos anteriores y después archiva viajes cerrados y Retrasos vencidos. Si falla una foto, conserva el snapshot original; se puede repetir sin duplicar filas u objetos.
6. Habilitar la aplicación y comprobar que `/api/health` devuelve `200` antes de reanudar tráfico. Después, volver a iniciar sesión con la cuenta de oficina y verificar una llegada, entrega, salida, consulta histórica y foto.
7. Confirmar la ejecución diaria de `/api/history/maintenance`. `vercel.json` programa las 09:05 UTC; el endpoint exige `Authorization: Bearer CRON_SECRET`. Si se hospeda fuera de Vercel, programar el mismo endpoint o `npm run history:retention` diariamente.
